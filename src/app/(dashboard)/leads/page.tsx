...
import { Zap, Mail, Phone, MapPin, Clock, CheckCircle2, MessageSquare, RefreshCw } from 'lucide-react'
      ...
      <div className="p-6 max-w-4xl mx-auto space-y-5">
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <div key={lead.id} className="bg-white rounded-2xl border border-slate-100 p-5">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4 flex-1 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2 flex items-center gap-2 flex-wrap">
                      <span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-xs">{lead.typeProjet}</span>
                      <span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-xs">{lead.puissance}</span>
                    </div>
                    {lead.message && (
                ...
                <div className="flex flex-col gap-2 flex-shrink-0">
                  <a href={`mailto:${lead.email}?subject=Votre demande de devis solaire`} className="btn-primary text-xs py-1.5 px-3 whitespace-nowrap">
                    Répondre
                  </a>
                  <select
                    value={lead.status}
